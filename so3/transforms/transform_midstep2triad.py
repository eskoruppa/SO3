...
def midstep2triad_lintrans(
    ...
    if len(groundstate_euler.shape) == 1:
        ...
        sqrt_rotmat  = euler2rotmat(0.5*Omega0) 
        if groundstate_definition != 'midstep':
            zeta0 = sqrt_rotmat.T @ zeta0
        
        zeta0_hat    = hat_map(zeta0)
        Hm = np.eye(6)
        Hm[3:,3:] = sqrt_rotmat
        ...
        Hm[3:,:3] = crosscoup
        Hm[:3,:3] = splittransform_group2algebra(Omega0)
        return Hm
