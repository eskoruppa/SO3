...
def se3_triadxrotmat_midsteptrans(tau1: np.ndarray, g: np.ndarray) -> np.ndarray:
    ...
    T1 = tau1[:3,:3]
    tau2 = np.eye(4)
    tau2[:3,:3] = np.dot(T1, R)
    # tau2[:3,:3] = T1 @ R
    tau2[:3,3] = tau1[:3,3] + np.dot(np.dot(T1, sqrt_rot(R)), g[:3,3])
...
def se3_triads2rotmat_midsteptrans(tau1: np.ndarray, tau2: np.ndarray) -> np.ndarray:
    ...
    T2 = tau2[:3,:3]
    R = np.dot(T1.T , T2)
    Tmid = np.dot(T1 , sqrt_rot(R))
    zeta = np.dot(Tmid.T, (tau2[:3,3]-tau1[:3,3]))
    # R = T1.T @ T2
    # Tmid = T1 @ sqrt_rot(R)
